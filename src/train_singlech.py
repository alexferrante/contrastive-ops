...
# enc/dec settings
parser.add_argument("--model", type=str, default="singlech") # str for enc/dec arch. 
...
parser.add_argument("--save_dir", type=str, default="/data/aferrant/contrastive-ops/logging_single_ch")
...
class SelectChannel(nn.Module):
    def __init__(self, channel_index: int):
        """
        Select a specific channel from the input tensor.
        Args:
    ...
    def forward(self, x: torch.Tensor):
        ...
        Returns:
            torch.Tensor: Output tensor with the selected channel of shape (N, 1, H, W).
        """
        if x.dim() != 4:
            raise ValueError("Input tensor must have 4 dimensions (N, C, H, W).")
        import pdb;pdb.set_trace()
        if self.channel_index < 0 or self.channel_index >= x.size(1):
            raise IndexError(f"Channel index {self.channel_index} is out of bounds for input with {x.size(1)} channels.")
        return x[:, self.channel_index:self.channel_index + 1, :, :]  # Keep channel dim for consistency
...
def train():
    ...
    data_param['transform'] = AugmentClass(transform, select_ch=2)
